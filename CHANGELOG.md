...
- **Changing default model logs out**: `PUT /api/settings/user/default-model` returned 401 when the JWT token was expired, triggering the frontend's global 401 handler → logout. Fixed by falling back to `setGlobalDefaultModel()` instead of returning 401 when unauthenticated (`0100d07d`).
