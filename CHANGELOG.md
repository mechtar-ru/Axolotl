...
#### PRISM Phase 3 — Operational Hardening (Plan 47)
- **Architecture documentation**: `backend/ARCHITECTURE.md` (191 lines) documenting operating envelope, single-user/single-schema execution contract, explicit non-goals, and when the architecture will break. Covers execution model, service topology, LLM routing, data flow, and change management patterns.
- **PipelineController extraction**: Pipeline endpoints (`/pipeline/build`, `/pipeline/execute`, `/pipeline/retry`, `/pipeline/cancel`, `/pipeline/status`, `/pipeline/default`) moved from `AgentController` (668→468 lines) to dedicated `PipelineController` (131 lines). `AgentController` no longer injects `PipelineService`.
- **NodeRouter strategy registry**: `NodeExecutionStrategy` interface with `supportedNodeType()`/`executeNode()`. `NodeRouter` collects all strategies via `List<NodeExecutionStrategy>` injection and builds a `@PostConstruct` map, replacing the hardcoded switch-case for `verifier`/`review`/`schemabuilder`/`draft` node types. Strategies auto-register by type string.
- **Circuit breaker per provider**: `CircuitBreakerWrapper` with CLOSED/OPEN/HALF_OPEN states, configurable failure threshold (default 3), success threshold (2), and open duration (30s). Integrated into `LlmService.chat()` and `LlmService.streamingChat()` — wraps each provider call. `getProvidersInfo()` outputs `circuitState` per provider. Full test coverage via `CircuitBreakerWrapperTest`.
- **WebSocket event replay**: `ExecutionWebSocketHandler` buffers events in per-schema `eventBuffer` when no session is connected. On `afterConnectionEstablished`, replays buffered events via `state_replay` envelope + individual events. Frontend `useWebSocket.ts` `onStateReplay` callback receives `{schemaId, eventCount}`.
- **Hop boundary validation**: `NodeOutputValidator` validates node outputs at each transition — checks null, missing `result` key, and type-specific patterns (blank agent output, verifier error prefix, null review/draft/source). Logs warnings per hop without blocking execution.
- **Feature flag system**: `FeatureFlagService` with 5 config-backed flags (`webhook`, `graph-query`, `tdd-pipeline`, `orchestration`, `experimental.draft-node`) read from `application.yml` (`axolotl.features.*`), overridable at runtime via `setOverride()`/`clearOverride()`. `FeatureFlagController` at `/api/features` with GET all, GET single, PUT toggle, DELETE override.
- **API key scoping**: `LlmService.resolveProvider()` checks `allowedModels` per API key — when a provider has key-scoped model restrictions, only the intersection of allowed and available models is exposed. Circuit breaker state surfaced per provider. (`<current>`)
