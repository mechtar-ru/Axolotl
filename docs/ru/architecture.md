...
flowchart LR
    Developer["Developer"]
    Axolotl["Axolotl<br/>Визуальная оркестрация AI-агентов"]
    LLM["LLM Provider APIs<br/>OpenAI / Anthropic / DeepSeek / Zen API"]
    Neo4j["Neo4j<br/>Графовая БД"]

    Developer -- "Использует (HTTP / WebSocket)" --> Axolotl
    Axolotl -- "Вызывает LLM (HTTPS)" --> LLM
    Axolotl -- "Читает/пишет (Bolt)" --> Neo4j
